...
const Navbar = () => {
  ...
  const menuLink =
    "flex items-center gap-3 px-4 py-2.5 text-sm text-gray-600 hover:text-gray-900 hover:-50 rounded-lg transition-colors";
          ...
          <div className="search-zone flex-1 max-w-md mx-auto relative hidden sm:block">
            <form onSubmit={handleSearchSubmit} className="relative">
              <input
                ...
                className="w-full pl-10 pr-4 py-2 -50 border border-gray-200 rounded-full text-sm outline-none focus:bg-white focus:border-sky-500 focus:ring-2 focus:ring-sky-100 transition-all"
              />
              <Search
                size={18}
                    ...
                    <div className="py-2 divide-y divide-gray-50">
                      {results.map((item) => (
                        <div
                          ...
                          className="flex items-center gap-3 px-4 py-2.5 hover:-50 cursor-pointer transition-colors"
                        >
        ...
        <div className="sm:hidden pb-3 search-zone relative">
          <form onSubmit={handleSearchSubmit} className="relative">
            <input
              ...
              onFocus={() => searchQuery.trim().length >= 2 && setShowSuggestions(true)}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-10 pr-4 py-2 -100 rounded-full text-sm outline-none focus:bg-white focus:ring-2 focus:ring-sky-100 border border-transparent focus:border-sky-300 transition-all"
            />
            <Search
              size={17}
              className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none"
                ...
                {loading ? (
                  ...
                  results.map((item) => (
                    <div
                      key={item.id || item._id}
                      onClick={() => handleSelectProduct(item.id || item._id)}
                      className="px-4 py-2.5 hover:-100 cursor-pointer text-sm text-gray-700 border-b border-gray-50 last:border-none"
                    >
                      {item.name}
                    </div>
                  ))
