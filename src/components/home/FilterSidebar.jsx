  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isSidebarOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-40 -900/40 backdrop-blur-sm"
          />
        )}
      </AnimatePresence>
      ...
      <motion.div
        ...
        className="fixed top-0 left-0 bottom-0 z-50 w-[280px] sm:w-[320px] bg-white shadow-2xl flex flex-col h-full border-r border-gray-200"
      >
        ...
        <button
          ...
          className="absolute top-30 left-full z-50 flex h-10 w-8 items-center justify-center rounded-r-xl bg-sky-800 text-white shadow-xl hover:-800 transition-colors"
        >
          <ChevronRight
            size={20}
        ...
        {/* Drawer Header */}
        <div className="flex items-center justify-between p-4 border-b border-gray-100 -50/50">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => onToggleSidebar(false)}
              className="p-1.5 rounded-full text-gray-400 hover:text-gray-700 hover:-200/60 transition-colors"
            >
              <X size={18} />
            </button>
          </div>
              ...
              <div className="mt-2.5 space-y-3">
                <div className="flex items-center gap-2">
                  <input
                    ...
                    placeholder="$ Min"
                    className="w-full px-3 py-1.5 rounded-lg border border-gray-200 text-xs outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500"
                  />
                      ...
                      <button
                        ...
                        className={`px-2.5 py-1 rounded-md border text-xs font-medium transition-colors ${
                          isActive
                            ...
                            : "border-gray-200 bg-white text-gray-600 hover:-50"
                        }`}
                      >
                        {range.label}
                      </button>
            ...
            {expanded.rating && (
              <div className="mt-2.5 space-y-1">
                {[4, 3, 2, 1].map((rating) => (
                  <label
                    key={rating}
                    className="flex items-center gap-2 cursor-pointer px-2 py-1.5 rounded-lg hover:-50 transition-colors"
                  >
                    <input
                      type="radio"
                      name="rating"
        ...
        {/* Drawer Footer */}
        <div className="p-4 border-t border-gray-100 -50/50">
          <button
            onClick={() => onToggleSidebar(false)}
...
const SectionHeader = ({ title, isOpen, onToggle }) => (
  <button
    type="button"
    onClick={onToggle}
    className="w-full flex items-center justify-between py-1.5 border-b border-gray-100 text-left select-none"
  >
...
const RadioOption = ({ label, checked, onChange, name }) => (
  <label className="flex items-center gap-2.5 cursor-pointer px-2 py-1.5 rounded-lg hover:-50 transition-colors text-xs font-medium text-gray-600">
    <input
      type="radio"
      name={name}
      checked={checked}
